...
function OurServices2() {
  const data:{id:number, title:string, img:any, text:any, help:any,keyEntertainment:any,moreText:any,moreTextHeader:any}[] = [
    { id:1,
...
<div className="md:flex px-[20px] items-start flex-wrap justify-between ">
          {data.map((item) => (
           <React.Fragment key={item.id}>
              <ServicesCard id={item.id} title={item.title}  text={item.text} img={item.img} keyEntertainment={item.keyEntertainment} help={item.help} moreText={item.moreText} moreTextHeader={item.moreTextHeader} />
           </React.Fragment>
          ))}
        </div>
