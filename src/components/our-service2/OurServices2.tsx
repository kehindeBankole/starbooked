...
function OurServices2() {
  const data:{title:string , text:any , img:any,help:any,keyEntertainment:any,moreText:any,moreTextHeader:any}[] = [
    {
      title: "Corporate Entertainment",
      text: "Maybe you would like a high profile musician such as Burna Boy to get your attendees out on the dance floor and create a sense of fun camaraderie. Or, maybe you’d like a comedian such as Basket Mouth to set a light and humorous tone for your evening, helping to break the ice for your attendees. Ultimately, your choice of entertainment will reflect the level of professionalism you put into planning your corporate event and—if done right—impress those in attendance beyond measure.",
      img: Talent2,
...
<div className="md:flex px-[20px] items-start flex-wrap justify-between ">
          {data.map((item, index) => (
           <React.Fragment key={index}>
              <ServicesCard title={item.title}  text={item.text} img={item.img} keyEntertainment={item.keyEntertainment} help={item.help} moreText={item.moreText} moreTextHeader={item.moreTextHeader} />
           </React.Fragment>
          ))}
        </div>
...
<Accordion expanded={expanded === `panel${index}`} onChange={handleChange(`panel${index}`)}>
            ...
            >
              ...
              <Typography className={`${styles.accordionText}`}>{item.moreTextHeader}</Typography>
            </AccordionSummary>
            <AccordionDetails>
              ...
              <Typography className={`${styles.accordionText}`}>
              <h2 className="capitalize font-bold mt-[10px]">Unsure? We Are Here to Help</h2>
          {item.help}
              </Typography>

              <Typography className={`${styles.accordionText}`}>
              <h2 className="capitalize font-bold mt-[10px]">Turn-Key Entertainment</h2>
          {item.keyEntertainment}
              </Typography>
...
{/* <ul style={{listStyleType:"circle"}}><li className="capitalize">Proposal</li><li className="capitalize">Engagement Party</li><li className="capitalize">Rehearsal Dinner</li><li className="capitalize">Wedding Reception</li><li className="capitalize">Wedding After Party</li></ul> */}